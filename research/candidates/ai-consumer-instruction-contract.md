...
### Resolution of the conflict with `tools/edit-replay/`, 2026-08-15

`tools/edit-replay/` was written nineteen minutes after this section and contradicts it. Its docstring argues that a repository's committed revisions are a population of **accepted** states, that live edits are **candidate** states, and that limits drawn from accepted states and applied to candidate states are too tight, so the chart fires on ordinary work in progress. Since the interrupt rate is this contract's own kill criterion, that is the criterion failing by construction rather than by measurement.

The objection is real and this section does not answer it. Three things narrow it, and together they make it a smaller decision than it looks.

**The disagreement is unmeasured, and the measurement is currently blocked.** Settling it means comparing the variance of a file's committed revisions against the variance of its candidate states. `edit-replay` exists to reconstruct the second population and currently reproduces **one anchor-to-anchor span in six** on `state_bounds.py`, with 28 divergences, because several sessions edit one tree. Until reconstruction is sound, neither position has evidence.

**The specification bands already answer acceptability, so the baseline decides less than assumed.** Good is defined by the thresholds, not by the center line. A baseline only answers whether something is moving. A file with no history can still be judged in-spec or out-of-spec from a single reading; it simply cannot be judged in-control or out-of-control.

**The fallback in this section is the common case, not the edge case.** Measured over 3,540 transcripts and 10,876 file-and-session pairs: the median number of writes to one file in one session is **one**, and 56% of files are written once and never again. Only 29% reach three writes and 7% reach ten. So "no control limits, insufficient history" is the operating mode for most work, and the choice of baseline population governs only the minority of files that both accumulate edits in a session and carry eight or more committed revisions.

**Standing position until measured.** This section's choice holds, with its claim weakened: the equivalence between a committed revision and a saved version is an assumption rather than an established fact, and `edit-replay` names the specific way it could be wrong. Anyone charting against these limits reports the assumption alongside the verdict. If the interrupt rate on real sessions exceeds the kill criterion, `edit-replay` was right and the population changes; that is the experiment, and it needs reconstruction fixed first.
