...
**The floor's derivation no longer holds, 2026-08-18.** Measured over all eight pinned repositories: json-c 0.608, junit4 0.568, Newtonsoft.Json 0.567, gson 0.543, libuv 0.534, RestSharp 0.517, `psf/requests` 0.511, `pallets/click` 0.366. Seven of the eight are above the floor, so it refuses a grade to almost the whole corpus, including libuv, whose 0.458 set it. That is the exact inverse of the sentence below. The paragraph below predicted movement in the other direction, that teaching the analyzer builtins would lower observed silence and bring the floor down with it; silence rose instead. `unmodeled_construct` accounts for 1,174 of 1,759 silent states across the corpus, 67 percent, and its top shapes are ordinary member reads rather than obscure code: an identifier in a C# member access 173 times, in a Java field access 142, in a C# expression-bodied member 91, and an attribute in a Python keyword argument 34. That is a list of rules the classifier does not have. Tracked as slop-audit-4b5. The number below has not been changed: a floor raised to fit whatever we currently cannot read measures our impatience, which is the objection this section already makes to 0.10.
