...
**The floor's derivation no longer holds, 2026-08-18.** libuv measures 0.534 today, so the floor refuses a grade to the very repository whose 0.458 set it, which inverts the sentence below. `psf/requests`, added to the pinned corpus on the same date, measures 0.511 and is refused for the same reason. The paragraph below predicted movement in the other direction, that teaching the analyzer builtins would lower observed silence and bring the floor down with it; silence rose instead. For `psf/requests` the dominant reason is `unmodeled_construct`, 27 of 45, naming ordinary Python shapes such as an attribute in a keyword argument, an attribute in an interpolation and a call in a for statement, which is a list of rules the classifier does not have rather than obscurity in the code. Tracked as slop-audit-4b5. The number below has not been changed: a floor raised to fit whatever we currently cannot read measures our impatience, which is the objection this section already makes to 0.10.
