...
- What happens to records collected under one tool version when the computation changes, as L1.18's did on 2026-08-15? The `tool` field allows them to be separated. Whether they should be pooled, and under what argument, is a question for whoever writes the calibration paper. **This got harder the same day.** A second change teaches the declaration classifier three kinds it had no rule for, and it moves the numbers a long way: libuv had 1,133 of 1,345 declarations unreadable, Newtonsoft.Json 678 of 1,360. Records either side of it are not the same measurement, so pooling them by indicator alone would mix two quantities under one name. Whoever writes the calibration paper has to split on `tool`, not merely record it.
- Should the first release restrict itself to the four indicators that are meaningful at file scope, measured in the MCP work, rather than all twenty? **Answered by what shipped.** The write-time hook of 2026-08-15 carries L1.16, L1.17 and a delegated L1.18, which is the set that survives file scope. Telemetry that covers more than the client can honestly measure would be collecting verdicts nobody computed.
