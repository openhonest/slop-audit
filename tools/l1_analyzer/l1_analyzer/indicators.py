...
# Conventional directories that hold dev/ops/entry-point scripts and data-seeding
# tooling rather than the library under test — the directory-level counterpart to
# the loose root-script rule below. `scripts/` is the near-universal home for
# entry-point and ops scripts (parallel to `tests/`); `seed`/`seeds` hold
# data-population scripts. Scoped out for the same reason a loose root script is:
# "not the code under test" (see card.py scoped.why). Every scoped file is
# disclosed in the bucket report, never silently skipped.
_TOOLING_DIRS = frozenset({"scripts", "seed", "seeds"})
...
def bucketed_paths(repo: Path, extensions: tuple[str, ...], extra_ignore: tuple[str, ...]) -> dict[str, Any]:
    ...
    for ext in extensions:
        for f in repo.rglob(f"*{ext}"):
            ...
            if reason in ("docs", "tooling", "root-script", "scripts"):
