...
and stops. Exactly one place decides, `indicators.compute_source_indicators`, and what it
decides is `n/a` with the reason printed, never a band. That is Typed Exceptions at the
Boundary applied to measurement: do not catch inside the business logic, let the function
raise, and let the boundary inspect the type and choose the output.
