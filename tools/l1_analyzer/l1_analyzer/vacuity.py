...
def _refuses(stmts: list[ast.stmt], refusing_calls: frozenset[str], scope: ast.AST) -> bool:
    ...
    # function did not, and that mismatch convicted the repair it exists to protect. Since
    # 2026-08-16 the sanctioned refusal in this package is `raise incomplete.refuse(...)`,
    # spelled as a raise precisely so it cannot be ignored by a caller, and every site written
    # that way was reported as an unrepaired empty-denominator path.
    #
