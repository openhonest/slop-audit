...
def test_a_profile_with_a_total_yields_the_statement_share_and_its_band():
    r = go_trace._coverage_verdict(_func_output("95.0"), True, 0, _RAN_OUTPUT, "go version go1.24.0 darwin/arm64", 300.0)
...
def test_the_details_disclose_that_go_publishes_statement_coverage_not_branch_coverage():
    ...
    r = go_trace._coverage_verdict(_func_output("72.0"), True, 0, _RAN_OUTPUT, "go1.24.0", 300.0)
    assert "statement coverage" in r["details"].lower()


def test_the_band_boundaries_follow_the_spec():
    def band(pct):
        return go_trace._coverage_verdict(_func_output(pct), True, 0, _RAN_OUTPUT, "go1.24.0", 300.0)["band"]
...
def test_a_failing_suite_that_still_wrote_a_profile_is_measured():
    ...
    r = go_trace._coverage_verdict(_func_output("64.0"), True, 1, "FAIL\tacme/widget\t0.3s", "go1.24.0", 300.0)
...
def test_a_timed_out_suite_is_named_rather_than_scored():
    r = go_trace._coverage_verdict("", False, 124, "timed out", "go1.24.0", 300.0)
