...
from l1_analyzer import dead_code, dead_code_corpus
...
def test_a_documentation_file_contributes_its_words_as_documentation():
    corpus = _empty()
    dead_code_corpus.classify_into(corpus, "README.md", ".md", b"see run_report for details")
...
def test_a_configuration_file_contributes_its_words_as_configuration():
    corpus = _empty()
    dead_code_corpus.classify_into(corpus, "ci.yml", ".yml", b"run: run_report --now")
...
def test_a_binary_file_contributes_nothing():
    ...
    corpus = _empty()
    dead_code_corpus.classify_into(corpus, "logo.png", ".png", b"\x89PNG\x00\x00run_report")
    assert corpus["words"]["config"] == set() and corpus["words"]["docs"] == set()


def test_source_is_parsed_rather_than_word_matched():
    corpus = _empty()
    dead_code_corpus.classify_into(corpus, "m.py", ".py", b"def run_report():\n    return 1\n")
    assert corpus["words"]["config"] == set()
...
def test_the_classifier_touches_nothing():
    ...
    source = pathlib.Path(dead_code_corpus.__file__).read_text()
...
def test_the_entry_points_a_package_manifest_declares_are_read_from_its_data():
    ...
    assert dead_code_corpus.entry_paths_in(data, repo / "pkg", repo) == {
        "pkg/lib/index.js", "pkg/lib/index.d.ts", "pkg/bin/tool.js"}


def test_a_manifest_declaring_no_entry_point_declares_none():
    assert dead_code_corpus.entry_paths_in({"version": "1.0.0"}, pathlib.Path("/r"),
                                    pathlib.Path("/r")) == set()
...
def test_a_bin_that_is_a_bare_string_is_not_read_as_a_map():
    ...
    assert dead_code_corpus.entry_paths_in({"bin": "tool.js"}, pathlib.Path("/r"),
                                    pathlib.Path("/r")) == set()


def test_the_metaprogramming_marker_is_read_from_the_text():
    assert dead_code_corpus.metaprogramming_in("class X\n  define_method(:go) { 1 }\nend\n")
    assert dead_code_corpus.metaprogramming_in("class X\n  def go\n    1\n  end\nend\n") == ""
