...
def test_a_crate_whose_suite_ran_nothing_is_not_banded():
    ...
    result = rust_trace._coverage_verdict((3, 0), 0, _TOOLCHAIN, 600.0, _NO_TESTS)
...
def test_a_crate_whose_suite_ran_is_measured_as_before():
    ...
    stands."""
    result = rust_trace._coverage_verdict((100, 66), 0, _TOOLCHAIN, 600.0, _TWO_TESTS)
...
def test_zero_regions_is_still_refused_on_its_own_terms():
    ...
    result = rust_trace._coverage_verdict((0, 0), 0, _TOOLCHAIN, 600.0, _TWO_TESTS)
    assert result["band"] == "n/a"
