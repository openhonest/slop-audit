...
`thread_surface.NO_SCANNER` is the string "n/a", and `cli._verdict_of` returns "n/a" when
...
def test_the_two_facts_do_not_share_a_string():
    assert cli.ABSENT_VERDICT != thread_surface.NO_SCANNER, \
...
def test_an_entry_with_no_verdict_reads_as_absent_not_as_no_scanner():
    assert cli._verdict_of({"value": 1, "band": "Healthy"}) == cli.ABSENT_VERDICT
    assert cli._verdict_of("not a dict at all") == cli.ABSENT_VERDICT
...
def test_a_scanner_less_language_still_reads_as_no_scanner():
    ...
    assert cli._verdict_of({"verdict": thread_surface.NO_SCANNER}) == thread_surface.NO_SCANNER


def test_both_facts_have_their_own_sentence_in_the_table():
    table = cli._UNMEASURED_THREAD_SURFACE
    assert thread_surface.NO_SCANNER in table
    assert cli.ABSENT_VERDICT in table
    assert table[cli.ABSENT_VERDICT] != table[thread_surface.NO_SCANNER], \
