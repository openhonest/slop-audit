...
returncode, sdk, 300.0)` and `_determinism_verdict(outcomes, sdk, 300.0)` take plain values and return the
...
def test_a_completed_run_yields_the_covered_share_and_its_band():
    r = csharp_trace._coverage_verdict((38, 40), 0, "dotnet 8.0.401", 300.0)
...
def test_a_failing_but_valid_run_is_still_measured():
    r = csharp_trace._coverage_verdict((7, 10), 1, "dotnet 8", 300.0)
...
def test_the_coverage_band_boundaries_follow_the_spec():
    def band(covered, valid):
        return csharp_trace._coverage_verdict((covered, valid), 0, "d", 300.0)["band"]
...
def test_a_timed_out_run_is_named_rather_than_scored():
    r = csharp_trace._coverage_verdict((38, 40), 124, "d", 300.0)
...
def test_a_run_that_wrote_no_cobertura_report_names_the_missing_collector():
    ...
    r = csharp_trace._coverage_verdict(None, 0, "d", 300.0)
    assert r["band"] == "n/a" and r["value"] == "n/a"
...
def test_branches_valid_of_zero_is_absent_not_zero_percent_and_carries_its_remedy():
    ...
    r = csharp_trace._coverage_verdict((0, 0), 0, "d", 300.0)
    assert r["band"] == "n/a" and r["value"] == "n/a"
...
def test_every_run_passing_is_healthy():
    r = csharp_trace._determinism_verdict([(0, _PASSED)] * 5, "dotnet 8.0.401", 300.0)
...
def test_the_detail_says_the_order_was_scheduler_varied_not_seed_controlled():
    ...
    r = csharp_trace._determinism_verdict([(0, _PASSED)] * 5, "d", 300.0)
...
def test_one_failing_run_is_not_healthy_and_the_run_carries_its_first_line():
    ...
    r = csharp_trace._determinism_verdict(outcomes, "d", 300.0)
...
def test_two_failing_runs_are_slop():
    ...
    assert csharp_trace._determinism_verdict(outcomes, "d", 300.0)["band"] == "Slop"


def test_at_most_three_failing_runs_are_named():
    r = csharp_trace._determinism_verdict([(1, _FAILED)] * 5, "d", 300.0)
...
def test_a_run_where_the_suite_never_executed_is_not_a_determinism_result():
    ...
    r = csharp_trace._determinism_verdict(outcomes, "dotnet 8", 300.0)
    assert r["band"] == "n/a" and r["value"] == "n/a"
...
def test_a_timed_out_run_stops_the_count_and_names_its_run():
    r = csharp_trace._determinism_verdict([(0, _PASSED), (0, _PASSED), (124, "")], "d", 300.0)
    assert r["band"] == "n/a" and r["value"] == "n/a"
