Feature: dead_code — finding unreachable statements and unreferenced definitions from syntax alone, and disclosing everything it cannot decide
  ...
  Scenario: _modules_named_by_a_dotted_string finds the files a string loads that nothing imports
    Given every dotted string the repository holds and the production files it has
    When _modules_named_by_a_dotted_string resolves each string against those files
    Then a module named as `pkg.plugin` in a subprocess argument is found, though no import statement anywhere leads to it
    But the string is resolved rather than pattern-matched, so a version number, a filename and a sentence ending in a full stop name nothing and excuse nothing

  Scenario: _loaded_reason says why a definition in a loaded module cannot be called dead
    Given the file a definition sits in and the set of modules something loads by name
    When _loaded_reason is asked about it
    Then a definition in a loaded module is undecidable, with the reason that a framework calls into it without an import
    But it is not excluded outright, because excluding these claims they all run and flagging them claims none of them does
