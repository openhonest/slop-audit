...
The slop audit is **not** a deep architectural review. It is **not** a complete remediation plan. It is **not** a recommendation about which AI tools to use or stop using. It overlaps with automated code-quality tools (SonarSource, CodeScene) on some mechanical indicators (dead code, duplication) but goes substantially further in two directions those tools do not cover: compliance-framework mapping (six frameworks, named with their evidence in `04-compliance-frameworks.md`) and finite testability measurement (mutable state ratio, decision-space coverage, test determinism). It also includes human-judgment layers (Layer 3 specified markers and Layer 4 architectural synthesis) that no automated tool can provide.
